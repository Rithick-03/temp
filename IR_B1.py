...
stop_words = set([
    "a", "am", "an", "and", "any", "are", "aren't", "as", "at", "be",
...
def custom_stem(word):
    ...
    This is a basic example and not as comprehensive as NLTK's PorterStemmer.
    """
    if len(word) > 2:
...
# --- Preprocessing Function (updated to use custom functions) ---
def preprocess(text):
    """
    Preprocesses a single text string by tokenizing, stemming,
...
def read_documents(file_path):
    """
    Reads documents and labels from a CSV file.
    Assumes the file has a header and two columns: 'text' and 'label'.
    Labels must be 'R' (Relevant) or 'NR' (Not Relevant).
    """
    try:
...
def run_bim_retrieval(docs, processed_docs, true_relevant_indices):
    ...
    # Map ground truth to string IDs
...
def run_vsm_retrieval(docs, processed_docs, true_relevant_indices):
    ...
    # Map ground truth to string IDs for evaluation
    true_relevant_docs_str = {f"d{i + 1}" for i in true_relevant_indices}
    SIM_THRESHOLD = 0
...
def main():
    ...
    docs, labels, ground_truth_indices = read_documents(file_path)
    if not docs:
